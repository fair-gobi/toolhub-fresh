...
export default function GPA() {
  ...
  const gpa = totalCredits? subjects.reduce((sum,s)=>sum + grades[s.grade]*s.credit,0) / totalCredits : 0;
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={()=>{localStorage.removeItem('gpa'); setSubjects([{name:'',credit:3,grade:'A'}])}} className="px-3 py-1 bg-red-100 rounded">Clear Saved</button>
        </div>
        {subjects.map((s,i)=>(
